...
preexist_raw <- read.csv("S:/RESLeonardJ_NickPondel/0_nick_final/1-raw_datasets/preexist.csv", header = TRUE)


#new dataframe with only needed columns
preexist <- data.frame("KEY_CASE" = preexist_raw$KEY_CASE,
                         "Q.text" = preexist_raw$PreExist.Question,
                         "A.text" = preexist_raw$PreExist.Answer)

preexist <- unite(preexist, preexist.QA, sep = " = ", -1, remove = TRUE)
preexist$KEY_CASE <- as.character(preexist$KEY_CASE)
preexist$preexist.QA <- as.character(preexist$preexist.QA)


#create corpus from data, thus creating duplicate IDs
corpus1 <- corpus(preexist, docid_field = "KEY_CASE", text_field = "preexist.QA")
...
names(shape1)[names(shape1) == "combined_text"] <- "preexist_text"
...
#export final CSV
write.csv(shape1, "S:/RESLeonardJ_NickPondel/0_nick_final/3-compiled_data/preexist_compiled.csv")