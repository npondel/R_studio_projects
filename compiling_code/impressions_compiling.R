...
impressions_raw <- read.csv("S:/RESLeonardJ_NickPondel/0_nick_final/1-raw_datasets/impressions.csv", header = TRUE)


#new dataframe with only needed columns
impressions <- data.frame("KEY_CASE" = impressions_raw$KEY_CASE,
                         "Q.text" = impressions_raw$QUESTIONTEXT,
                         "A.text" = impressions_raw$ANSWER)

impressions <- unite(impressions, impressions.QA, sep = " = ", -1, remove = TRUE)
impressions$KEY_CASE <- as.character(impressions$KEY_CASE)
impressions$impressions.QA <- as.character(impressions$impressions.QA)


#create corpus from data, thus creating duplicate IDs
corpus1 <- corpus(impressions, docid_field = "KEY_CASE", text_field = "impressions.QA")
...
names(shape1)[names(shape1) == "combined_text"] <- "impressions_text"
...
#export final CSV
write.csv(shape1, "S:/RESLeonardJ_NickPondel/0_nick_final/3-compiled_data/impressions_compiled.csv")